...
Data that is shared can not be properly aligned to the tasks, which means all tasks have to be able to access the data at any time. Task data access in the PC-stable aglorithm is nearly randomized on correlation matrix in level 1 and following, because the separation sets can be build from all existing vertices. 
...
The maximum iteration count per row can be calculated through dividing the CI-Test count (or separation set count) for a given edge by the number of threads working in parallel on that edge. In the implementation done for this thesis Nvidia GPUs are used and in levels 1 and following, each edge is processed in parallel by a thread block. The thread block size is heuristically chosen as a multiple of the warp size, because threads in a warp execute instructions synchronized. In the case of this thesis a block in levels 1 and following is defined as 64 threads. Using those 64 thread-sized block, 64 CI-tests are done in parallel, and the CI-test count has to be divided by 64 to calculate the maximum iteration count.
...
If the maximum execution time of every edge in a row is proportional to the maximum possible iterations, the execution time of a single row can be estimated by the row length, because with a larger row length, the CI-Test coun also grows.
