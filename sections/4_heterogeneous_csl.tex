...
Data that is only read during the task execution can be copied in both the CPU and the GPU memory to prevent page migrations and boost memory access performance. In the case of gaussian CI-Tests, a global covariance matrix is only read through the whole algorithm. For performance optimization, this matrix is stored in both the CPU and the GPU memory.
