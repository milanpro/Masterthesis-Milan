...
As can be seen in \ref{alg:pcstable} the starting point of the algorithm is a set of vertices that represent the given variables. The PC-Algorithm incrementally learns a skeleton in each level. Starting with a separation set of size 0, independence tests for every edge pair is done. Every edge between any vertice combination exist in level 0, therefore the independence test count can be calculated as $|P_0| = \frac{(|V| - 1) * |V|}{2}$. In subsequent level iterations separation sets are non empty and the existence of an edge cannot be assumed.
